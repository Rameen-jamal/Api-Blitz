...
import { Users, Target, FileText, TrendingUp, Trophy, Activity } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  if (loading) {
    return (<div className="flex-1 flex items-center justify-center min-h-screen"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-green-500"></div></div>);
  }

  const statCards = [
    { label: 'Total Teams', value: stats.totalTeams, icon: Users, gradient: 'from-green-500/20 to-emerald-500/10', iconColor: 'text-green-400', borderColor: 'border-green-500/20' },
    { label: 'Challenges', value: stats.totalChallenges, icon: Target, gradient: 'from-emerald-500/20 to-teal-500/10', iconColor: 'text-emerald-400', borderColor: 'border-emerald-500/20' },
    { label: 'Active', value: stats.activeChallenges, icon: Activity, gradient: 'from-lime-500/20 to-green-500/10', iconColor: 'text-lime-400', borderColor: 'border-lime-500/20' },
    { label: 'Submissions', value: stats.totalSubmissions, icon: FileText, gradient: 'from-teal-500/20 to-cyan-500/10', iconColor: 'text-teal-400', borderColor: 'border-teal-500/20' },
  ];
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8 animate-fade-in-up">
        <h1 className="font-teko text-4xl font-bold text-white tracking-wide">SITUATION REPORT</h1>
        <p className="text-gray-600 text-sm mt-0.5">Overview of competition operations</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statCards.map(({ label, value, icon: Icon, gradient, iconColor, borderColor }, index) => (
          <div key={label} className={`glass-card rounded-xl p-5 animate-fade-in-up stagger-${index + 1} border ${borderColor}`}>
        ...
        <table className="admin-table">
          ...
          <tbody>
            {leaderboard.map((entry) => (
              <tr key={entry.rank}>
                <td className="text-gray-500 font-mono text-sm">{entry.rank}</td>
                <td className="text-white font-medium text-sm">{entry.teamName}</td>
                <td className="text-center text-gray-500 text-sm">{entry.challengesSolved}</td>
                <td className="text-center text-gray-600 text-xs">
                  {entry.lastSolvedAt ? new Date(entry.lastSolvedAt).toLocaleTimeString() : '-'}
                </td>
                <td className="text-right font-mono font-bold text-green-400 text-sm text-glow">{entry.score}</td>
              </tr>
            ))}
            {leaderboard.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-gray-600">No teams yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
