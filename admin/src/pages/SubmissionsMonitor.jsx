    ...
    <div className="p-8">
      <div className="mb-6 animate-fade-in-up">
        <h1 className="font-teko text-3xl font-bold text-white tracking-widest flex items-center gap-2">
          <FileText className="h-6 w-6 text-purple-400" />INTEL FEED
        </h1>
        <p className="text-purple-300/50 text-xs mt-0.5 tracking-wide">Monitor all flag submissions and adjust scores</p>
      </div>

      {/* Filters */}
      <div className="glass-card-static rounded-xl p-4 mb-6 animate-fade-in-up stagger-1">
        <div className="flex items-center gap-4 flex-wrap">
          <Filter className="h-4 w-4 text-purple-400/50" />
      ...
      <div className="glass-card-static rounded-xl p-5 mb-6 animate-fade-in-up stagger-2">
        <h3 className="font-teko text-lg font-semibold text-white tracking-widest mb-3">MANUAL SCORE OVERRIDE</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {teams.map(team => (
            <div key={team._id} className="flex items-center gap-2">
              <span className="text-sm text-purple-200 w-28 truncate">{team.teamName}</span>
              <span className="text-xs text-purple-400/50 font-mono">({team.score})</span>
              <input type="number" value={scoreAdjust[team._id] || ''} onChange={(e) => setScoreAdjust(prev => ({ ...prev, [team._id]: e.target.value }))}
                placeholder="New" className="flex-1 px-2 py-1.5 glass-input rounded text-white text-sm font-mono focus:outline-none placeholder-purple-900/50" />
              <button onClick={() => adjustScore(team._id)} className="p-1.5 btn-purple rounded transition-colors" title="Save">
      ...
      <div className="glass-card-static rounded-xl overflow-x-auto animate-fade-in-up stagger-3">
        <table className="admin-table">
          <thead>
            <tr><th>Team</th><th>Challenge</th><th>Submitted Flag</th><th className="text-center">Result</th><th className="text-right">Time</th></tr>
          </thead>
          <tbody>
            {submissions.map((sub) => (
              <tr key={sub._id}>
                <td className="text-white text-sm font-medium">{sub.teamId?.teamName || 'Unknown'}</td>
                <td className="text-purple-200/60 text-sm">{sub.challengeId?.title || 'Unknown'}</td>
                <td className="text-purple-300/50 text-sm font-mono max-w-xs truncate">{sub.submittedFlag}</td>
                <td className="text-center">
                  {sub.isCorrect
                    ? <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-teal-500/15 text-teal-400 rounded-full text-xs border border-teal-500/20"><Check className="h-3 w-3" />Hit</span>
                ...
                <td className="text-right text-purple-300/40 text-xs font-mono">{new Date(sub.attemptedAt).toLocaleString()}</td>
              </tr>
            ))}
            {submissions.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-purple-400/30">No submissions yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
